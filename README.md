...
- Multiclass Log Loss and Brier Score for the original (uncalibrated) model
- A **cross-validated method-selection step** comparing **Platt Scaling (sigmoid)** and **Isotonic Regression**
- A single, final evaluation of the selected method on the held-out test set

### Calibration Method Selection

To avoid selecting a calibration method based on its performance on the held-out test set — which would leak test-set information into the model-selection process — Platt Scaling and Isotonic Regression were instead compared using **5-fold cross-validation on the training data only**. For each method, a `CalibratedClassifierCV` wrapper was cross-validated and the out-of-fold (OOF) Log Loss was recorded; the method with the lower OOF Log Loss was selected for a single, final evaluation on the untouched test set.

| Method | OOF Log Loss (Cross-Validated) |
|---|---:|
| Platt Scaling (Sigmoid) | 0.2981 |
| **Isotonic Regression** | **0.2729** |

**Isotonic Regression** achieved the lower out-of-fold Log Loss and was selected as the calibration method to be evaluated on the test set.

### Final Test-Set Results

With Isotonic Regression selected via cross-validation, a single calibrated model was fit on the full training set and evaluated once on the held-out test set — the first and only point at which this test data was used for calibration evaluation.

| Model | Multiclass Log Loss | Multiclass Brier Score |
|---|---:|---:|
| Original XGBoost (Uncalibrated) | 0.2386 | 0.1366 |
| **Isotonic Regression (Selected)** | **0.2375** | **0.1362** |

Lower values indicate better probability performance. The isotonic-calibrated model achieved a marginally lower Log Loss and Brier Score than the original, uncalibrated XGBoost model on this held-out test set.

### Decision

Since the improvement from Isotonic Regression is small, the **original, uncalibrated XGBoost model was retained for deployment** for simplicity and consistency. The calibration experiments — including the cross-validated method comparison — are documented as post-training validation analyses, without changing the production pipeline.
...
6. **Probability calibration provided no meaningful benefit** — Isotonic Regression was selected over Platt Scaling via cross-validation (avoiding any test-set leakage during selection) and matched the original model within a marginal difference on the held-out test set, so the original XGBoost probabilities were retained.
...
**Live Demo:**  
https://multiclass-sleep-disorder-classification-y2ol.onrender.com
