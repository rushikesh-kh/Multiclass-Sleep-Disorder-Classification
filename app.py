...
# Fall back to model.py's schema for compatibility with older artifacts.
...
# ------------------------------------------------------------------
# Extract categories from the fitted OneHotEncoder
# ------------------------------------------------------------------

def _trained_categories(column):
    """
    Return the categories learned by the fitted model encoder.
...
for field, rule in VALIDATION_SCHEMA.items():
    ...
    # Prefer the actual categories learned by the model.
...
# These ranges are used only for the UI wellness scoring.
...
def validation_rules():
    ...
    The frontend can use this endpoint to stay synchronized
    with the model's validation schema.
    """
...
def score_vital(key, value):
    ...
    This score is for UI presentation only and is not
    part of the ML prediction.
    """
...
def build_factors(
    ...
    These factors are explanatory UI rules.
    They are NOT the internal feature importance
...
def predict():
    ...
    try:
        ...
        # Keep internal exception details out of the
